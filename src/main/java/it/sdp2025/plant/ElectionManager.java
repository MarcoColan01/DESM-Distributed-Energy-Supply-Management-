...
public class ElectionManager {
    ...
    public synchronized void checkPendingRequests() {
        if (!busy && !isProducing && !requestBuffer.isEmpty()) {
            ...
            do {
                ...
                if (nextRequest != null && processedRequests.contains(nextRequest.getTimestamp())) {
            ...
            } while (nextRequest == null && !requestBuffer.isEmpty());
    ...
    public synchronized void handleElection(@NotNull PlantNetwork.ElectionMessage message) {
        ...
        // Marca questa richiesta come processata (qualcuno ha avviato un'elezione)
        processedRequests.add(timestamp);
        ...
        if (!busy) {
            // Se sto producendo, non posso partecipare a questa elezione
            if (isProducing) {
    ...
    public synchronized void productionFinished() {
        ...
        // MA solo dopo un piccolo delay per evitare race condition
        new Thread(() -> {
            try {
                Thread.sleep(50); // Piccolo delay per permettere la propagazione dei messaggi
                synchronized (this) {
